...
public class Back {
     ...
     */
    private final ArrayList<Comisie> listaComisii; //SE ADAUGA STUDENTII LA COMISII
     ...
     */
    public Back(ArrayList<Student> listaStudenti, ArrayList<Profesor> listaProfesori,  ArrayList<Comisie> listaComisii, long nrSecunde) {
            ...
            for (Profesor profesor : listaProfesoriCuElevi) {
                if (student.getIdProf() == profesor.getId()) {
                    ...
                    for (Comisie comisie : listaComisii)
                        if (comisie.getIdComisie() == student.getIdComisie())
                            comisie.addStudent(student);
                    break;
                }
                i++;
            }
    ...
    public void adaugaLaComisiiStudentiiCuProfesoriInEle() {
        for (Comisie comisie : listaComisii) {
            int i = 1;
            for (Profesor profesor : listaProfesoriCuElevi) {
                if (comisie.inComisie(profesor.getId())) {
                    inComisie[i] = true;
                    comisiaProfesorului[i] = comisie.getIdComisie();
                }
                i++;
            }
        }
        ...
        for (Comisie comisie : listaComisii) {
            for (Student student : listaStudenti)
                if (comisie.inComisie(student.getIdProf()))
